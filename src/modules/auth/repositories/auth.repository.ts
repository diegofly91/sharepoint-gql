...
export class AuthRepository {
    ...
    async getSiteData(token: string, site?: string): Promise<ISite> {
        try {
            const url = site
                ? `https://graph.microsoft.com/v1.0/sites/${this.configService.get('TENANT_URL')}:/sites/${site}`
                : `https://graph.microsoft.com/v1.0/sites/${this.configService.get('TENANT_URL')}`;
